...
""".execute.apply()
  ...
  def createOrder(externalId:String):OptionT[LogsWriter, OrderRef] = {
    println(s"Creating order by externalId = $externalId")
    val newId = sql"insert into tbl_order (external_id) values ($externalId)".updateAndReturnGeneratedKey.apply()
    OrderRef(newId) loggedWith Logs.simple(s"Created order $externalId")
  }

  def writeTx(order:OrderRef, txType: TransactionType, amount: Money): OptionT[LogsWriter, Seq[Transaction]] = {
    ...
    val tx = Transaction(order, txType, amount)
    Seq(tx) loggedWith Logs.simple(s"Inserterd tx $tx")
  }
