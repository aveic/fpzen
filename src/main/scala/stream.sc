object streamz {
  ...
  sealed trait Stream[+A] {
    ...
    def flatMap[B](f: A => Stream[B]):Stream[B] = foldRight(Stream.empty[B])(f(_) append _)
    ...
    def scanRight[B](z:B)(f: (A, => B) => B):Stream[B] = foldRight((Stream.apply(z), z))( (el, accH) => {
      val (streams, acc) = accH
      val acc2 = f(el, acc)
      (Stream.cons2(acc2, streams), acc2)
    })._1
  }
...
Stream(1,2,3,4).exists(_ == 4)
Stream(1,2,3,4).exists(_ == 4)
Stream(1,2,3,4).takeWhile(_ <= 2).toList
...
Stream(1,2,3,4,5).takeViaUnfold(4).toList
Stream(1,2,3,4,5).takeWhileViaUnfold(_ < 4).toList
Stream(1,2,3,2,5).takeWhileViaUnfold(_ > 1).toList
Stream(1,2,3,2,5).mapViaUnfold(_ * 2).toList

Stream(1,2,3,4,5).zipWithViaUnfold(Stream("a", "b", "c")){_.toString + _}.toList
Stream(1,2,3,4,5).zipAllViaUnfold(Stream("a", "b", "c")).toList

Stream(1,2,3,5,6,1,2,3).startsWith(Stream(1,2,43))
